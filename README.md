...
4. **啟動開發服務器**
   ...
   # HTTPS模式（建議用於Live API功能）
   npm run start-https
   ```
   
   **⚠️ 重要提醒**: Live API 功能需要 HTTPS 環境才能正常運作，建議使用 `npm run start-https`
...
#### 修改 System Prompt
如果您想要自定義 AI 的行為和回應風格，可以修改 system prompt：

**檔案位置**: `src/utils/system-prompt-generator.ts`
...
**修改語調設定**: 在同一檔案中的 `getTonePrompt` 函數，您可以：
- 修改現有語調的描述
- 添加新的語調選項
- 調整語調對應的提示語句

**注意**: 修改後需要重新啟動開發服務器才會生效。
...
# 開發服務器
npm start              # HTTP模式
npm run start-https    # HTTPS模式（建議）
...
A: 
- 確保瀏覽器已授與麥克風權限
- 使用 HTTPS 模式啟動（`npm run start-https`）
- 只有 Live API 模式才支援語音輸入
...
- 在設定中檢查 TTS 模型和語音設定
- 手動點擊一次任意地方來啟用音頻
- 確保有網路連接以訪問 Gemini TTS API

**Q: 模式無法切換？**
A: 
- 確保沒有活躍的 Live API 或 LLM+TTS 連接
- 點擊斷開連接按鈕後即可切換
- 系統會自動解鎖切換功能
